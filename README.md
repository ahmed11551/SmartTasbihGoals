...
**Вариант 1: С внешней БД (рекомендуется для продакшена)**

Если у вас уже есть внешняя база данных (например, в облаке):

```bash
# Создать .env файл с DATABASE_URL
echo "DATABASE_URL=postgresql://user:password@host:port/database" > .env

# Запустить только приложение (без локальной БД)
docker-compose -f docker-compose.prod.yml up -d

# Приложение доступно на http://localhost:5001
```

**Вариант 2: С локальной БД (для разработки)**

Если нужна локальная БД в Docker:

```bash
# Запустить приложение + локальную БД
...
**Преимущества Docker:**
- Гибкость: можно использовать внешнюю БД или локальную
- Не требует отдельной настройки БД (если используете локальную)
